...
int Shader::initShaderProgram(const char* vert, const char* frag) {
...
void Shader::drawObjectInstaced(const Object3D& obj) {
    ...
    if(obj.options->positions.size()) {
        unsigned int positionsVBO;
        glGenBuffers(1, &positionsVBO);
        glBindBuffer(GL_ARRAY_BUFFER, positionsVBO);
        glBufferData(GL_ARRAY_BUFFER, obj.options->positions.size() * sizeof(glm::vec3), &obj.options->positions[0], GL_STATIC_DRAW);
    ...
    glBindVertexArray(VAO);
    glDrawArraysInstanced(GL_TRIANGLES, 0, obj.model->vertices.size(), obj.options->positions.size());

    glDeleteVertexArrays(1, &VAO);
    glDeleteBuffers(1, &VBO);
}

Shader::Shader(GLFWwindow *window, Camera* camera) {
    this->camera = camera;
    this->window = window;

    selectShader = initShaderProgram("shaders/shader.vert", "shaders/shader.frag");
